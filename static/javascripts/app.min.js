...
(function(){$(document.body).hasClass("home")&&($.ajaxSetup({data:{_xsrf:getCookie("_xsrf")}}),$("tr.room").delegate("a.danger","click",function(d){d.preventDefault();var e=$(this);confirm("Are you sure to delete this room? All the messages and files in this room will also be deleted")&&$.post("/rooms/"+e.data("room")+"/delete",function(){e.parents("tr.room").fadeOut()})}))})();
(function(){function d(){for(var a=arguments[0]||"",b=1;b<arguments.length;b++)a=a.replace(RegExp("\\{"+(b-1)+"}","gim"),arguments[b]);return a}function e(a){var b=".bmp,.gif,.jpg,.jpeg,.png,.tif".split(","),c=a.match(/(https?:\/\/.*(?=&quot;|&#39;))|[^\s\t]{0}(http:\/\/[^\s\t]*)/);if(c!==null&&c!==void 0){for(var i=[],f=0;f<c.length;f++)c[f]!==void 0&&c[f]!==null&&i.indexOf(c[f])===-1&&i.push(c[f]);for(f=0;f<i.length;f++)var c=i[f],e=c.match(/\.[^\.]+$/),a=e!==void 0&&e!==null&&b.indexOf(e[0])!==
...
300,200,a)}if(window.M){$("#room-menu a").removeClass("active");$("#room-menu a."+M.active_menu).addClass("active");var k=$("#new_message"),g=k.find("textarea"),j=function(a){var b=$("<tr>");b.append($("<td>").addClass("user").text(a.user_name));var c=$("<td>");if(a.type=="image"){var d=$("<a>").attr({href:a.url,target:"_blank"});d.append($("<img>").attr("src",a.thumb_url));c.append(d)}else a.type=="file"?c.append($("<a>").attr({href:a.url,target:"_blank"}).text(a.url)):a.type=="text"?c.html(e(a.content)):
...
$(this).val("")):$(this).val()!==""&&k.submit()});var h=function(){$("html, body").animate({scrollTop:$(document).height()},"slow")};$("#messages").find("tr.text td").each(function(a,b){b.innerHTML=e(b.innerHTML)});g.focus();setTimeout(h,50);var l=function(a){return(a=document.cookie.match("\\b"+a+"=([^;]*)\\b"))?a[1]:void 0};window.uploader=new plupload.Uploader({runtimes:"html5,flash",browse_button:"select_files",container:"upload_container",max_file_size:"10mb",url:"/rooms/"+M.room.id+"/upload",
...
$(this).addClass("active")});$(document.body).bind("end.pjax",function(){$("#messages").find("tr.text td").each(function(a,b){b.innerHTML=e(b.innerHTML)});g.focus();$("#messages").length>0&&setTimeout(h,50)})}})();